...
TOKEN=`mysql -uroot -proot portal -s -N -e "select str_value from config where config_id='config_console_token'";`
HOST=`mysql -uroot -proot portal -s -N -e "select str_value from config where config_id='server_host_port'";`

STORAGE=`df | grep xvda | awk '{ print $4 }'`
