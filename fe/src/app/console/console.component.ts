...
export class ConsoleComponent implements OnInit {
  ...
  public background: string = "radial-gradient(circle, rgba(190, 225, 252, 1) 0%, rgba(0, 0, 0, 1) 100%)";
  ...
  public devicesOptions: ChartOptions = { elements: { line: { borderWidth: 1 }, point: { radius: 0 } }, scales: { xAxes: [{ gridLines: { display: false } }], yAxes: [{ ticks: { min: 0, stepSize: 128 }}]}};
  public certsOptions: ChartOptions = { elements: { line: { borderWidth: 1 }, point: { radius: 0 } }, scales: { xAxes: [{ gridLines: { display: false } }], yAxes: [{ ticks: { min: 0, stepSize: 128 }}]}};
  ...
  onLogin() {
    this.consoleService.getStats(this.loginToken).then(() => {
      ...
      this.consoleService.getStats(this.token).then(s => {
        ...
        for(let i = 0; i < s.length; i++) {
          ...
          this.devices.unshift(s[i].devices);
          this.certs.unshift(s[i].certs);
        ...
        if(this.mode == "certs") {
          this.lineChartData = [ { data: this.certs } ];
          this.lineChartOptions = this.certsOptions;
        }
        if(this.mode == "devices") {
          this.lineChartData = [ { data: this.devices } ];
          this.lineChartOptions = this.devicesOptions;
        }
